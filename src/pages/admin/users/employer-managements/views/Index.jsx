...
export default function Index() {
  ...
  // Gọi API khi component mount và khi thay đổi tham số phân trang hoặc từ khóa tìm kiếm
  useEffect(() => {
  ...
  const handleSearch = () => {
    // Lấy giá trị từ ref thay vì state
            ...
            <table className="w-full border-collapse table-fixed">
              <thead>
                <tr className="bg-muted/40 border-b border-t">
                  ...
                  <th className="w-[31%] py-3.5 px-4 text-left font-medium text-xs uppercase tracking-wider text-foreground/70">
                    Email
                  </th>
                  <th className="w-[17%] py-3.5 px-4 text-left font-medium text-xs uppercase tracking-wider text-foreground/70">
                    Số dư tài khoản
              ...
              <tbody>
                {paginatedData.items && paginatedData.items.length > 0 ? (
                  paginatedData.items.map((employer, index) => (
                    <tr
                      key={employer.userId}
